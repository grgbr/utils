...
utime_tspec_after(const struct timespec *fst, const struct timespec *snd)
{
	return (utime_tspec_cmp(fst, snd) > 0);
}

static inline bool __utime_nonull(1, 2) __pure __nothrow
utime_tspec_after_eq(const struct timespec *fst, const struct timespec *snd)
{
	return (utime_tspec_cmp(fst, snd) >= 0);
}

static inline bool __utime_nonull(1, 2) __pure __nothrow
utime_tspec_before(const struct timespec *fst, const struct timespec *snd)
{
	return (utime_tspec_cmp(fst, snd) < 0);
}

static inline bool __utime_nonull(1, 2) __pure __nothrow
utime_tspec_before_eq(const struct timespec *fst, const struct timespec *snd)
{
	return (utime_tspec_cmp(fst, snd) <= 0);
}
